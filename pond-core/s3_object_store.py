...
class S3ObjectStore:
    ...
    def compare_and_set_path(self, path: str,
        ...
        Uses S3 conditional PUT:
          - If expected_hash is None: If-None-Match: "*" (only succeed
            if the key doesn't exist)
          - If expected_hash is set: read the current ETag, then PUT
            with If-Match: "{etag}". This is two round trips but
        ...
        if expected_hash is None:
            ...
            try:
                self._client.put_object(
                    ...
                    Body=json.dumps({"hash": new_hash}).encode(),
                    IfNoneMatch="*",
                )
                with self._lock:
                    self.stats["puts"] += 1
        ...
        # expected_hash is set: read current ETag, then conditional PUT
        try:
            head = self._client.head_object(Bucket=self._bucket, Key=key)
            current_etag = head["ETag"].strip('"')
            # Verify current hash matches expected
            ...
            # Path doesn't exist but expected_hash was set — fail
            with self._lock:
                self.stats["gets"] += 1
            return False
        ...
        try:
            ...
            # ETag changed (concurrent writer won)
            with self._lock:
                self.stats["gets"] += 1
            return False
