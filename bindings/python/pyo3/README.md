# pond-python — PyO3 bindings to pond-core

This crate produces the Python extension module `pond_rust.so` (named
`pond_rust` on the Python side). It depends on `pond-core` for all the
PND2 encode/decode logic and adds PyO3 on top to expose it to Python.

## Build

From the workspace root (`pond-rust/`):
...
# → /path/to/pond-rust/target/release/pond_rust.so
```
...
## What's here vs. `pond-core`

| Feature | `pond-core` (pure Rust) | `pond-python` (this crate) |
|---------|------------------------|-----------------------------|
...
Python bindings have full coverage. Future work: extend `pond-core`'s
