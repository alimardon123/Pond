...
def main():
    ...
    try:
        ...
        # Both FeatureStoreLens and LakehouseLens now use ProllyTreeIndex
        # storage, so the Lakehouse Lens can read the collection directly
        # via its read_table() method (which reads row groups from the
        # ProllyTreeIndex and concatenates them).
        features_table = lh.read_table("user_features")
        ...
        branch_table = lh.read_branch("user_features", "lh_dev")
        ...
        # Walk back to the original commit using inherited history()
        # (handles both binary ProllyLensBase commits and legacy JSON commits).
        history = fs.history("user_features")
        original_commit = history[-1]["hash"]  # last in history = first commit
        ...
        old_table_lh = lh.read_table("user_features", commit_hash=original_commit)
        ...
        new_table = lh.read_table("user_features")
        ...
        # Lakehouse Lens can walk the same commit chain via inherited history()
        lh_history = lh.history("user_features")
        lh_history_count = len(lh_history)
        ...
        # Check that the merge commit has 2 parents via inherited history()
        history = fs.history("user_features")
        latest = history[0]
