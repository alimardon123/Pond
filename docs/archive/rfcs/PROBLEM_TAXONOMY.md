...
**Action:** implement the optimization at the Lens level (skip pointers,
...
Lens author has to write too much boilerplate or reinvent the same
patterns repeatedly.

**Examples:** every View reimplements the Tree+Commit pattern; no shared
index library; no standard serialization format.

**Action:** create shared libraries (Lens-level, not kernel-level).
Document common patterns in the Lens Author's Guide.
...
| Finding 5a (time travel O(N)) | **Performance** | Needs Lens-level skip pointers |
...
| Index friction (5 Views) | **Ergonomics** | Needs shared index library (Lens-level) |
