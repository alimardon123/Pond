...
sys.path.insert(0, os.path.join(SCRIPT_DIR, "..", "pond-labs"))
...
def test_bidirectional(kernel, lh, fs):
    ...
    fs_table = pa.parquet.read_table(reader)
    ...
    parquet = kernel.read(commit["parquet"])
    reader = pa.BufferReader(parquet)
    lh_table = pa.parquet.read_table(reader)
...
def test_merge_safe(kernel, lh, fs):
    ...
    parquet = kernel.read(commit["parquet"])
    reader = pa.BufferReader(parquet)
    merged = pa.parquet.read_table(reader)
...
def test_time_travel_safe(kernel, lh, fs):
    ...
    # FeatureStore reads the same commit (via raw kernel)
    commit = json.loads(kernel.read(first_commit))
    parquet = kernel.read(commit["parquet"])
    reader = pa.BufferReader(parquet)
    old_fs = pa.parquet.read_table(reader)
